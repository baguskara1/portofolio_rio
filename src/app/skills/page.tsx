...
const skills = [
  ...
  { name: "Next.js", level: 85, icon: <SiNextdotjs className="w-6 h-6" />, color: "bg-white" },
];
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <AnimatedSection className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
            Technical <span className="text-pink-400">Skills</span>
          </h2>
        ...
        <div className="mb-20">
          <h3 className="text-xl font-bold text-white mb-8 text-center">Programming Languages</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {skills.map((skill, index) => (
              <AnimatedSection key={skill.name} delay={index * 0.05} className="bg-gray-900 p-6 rounded-xl shadow-sm border border-gray-800 hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
        ...
        {/* Tools & Frameworks */}
        <div>
          <h3 className="text-xl font-bold text-white mb-8 text-center">Tools & Frameworks</h3>
          <div className="flex flex-wrap justify-center gap-4">
            {tools.map((tool, index) => (
              <AnimatedSection key={tool.name} delay={index * 0.05} className="flex items-center gap-2 px-6 py-3 bg-gray-900 border border-gray-800 rounded-full text-gray-300 font-medium shadow-sm hover:shadow-md hover:border-pink-500/50 hover:-translate-y-1 transition-all cursor-default animate-float" style={{ animationDelay: `${index * 0.2}s` }}>
                <span className="text-pink-400">{tool.icon}</span>
                {tool.name}
              </AnimatedSection>
            ))}
          </div>
