        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => (
            <AnimatedSection
              key={project.title}
              delay={index * 0.05}
              className="group bg-gray-950 rounded-2xl shadow-sm border border-gray-800 overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-300"
            >
              ...
              <div className="p-6">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.tags.map(tag => (
                    <span key={tag.name} className="flex items-center gap-1 text-xs px-2 py-1 bg-gray-800 text-gray-300 rounded-md border border-gray-700">
                      {tag.icon}
                      {tag.name}
                    </span>
                  ))}
                ...
                <div className="flex items-center space-x-4 pt-4 border-t border-gray-800">
                  <a
                    ...
                    className="flex items-center gap-2 text-sm text-gray-400 hover:text-pink-400 transition-colors font-medium"
                  >
